...
controller_plugin="$(python_with_ros_env "$SLAM_PARAMS_FILE" <<'PY'
import sys
...
bt_report="$(python_with_ros_env "$SLAM_PARAMS_FILE" "$SCRIPT_NAME" <<'PY'
import os
import sys
from pathlib import Path

cfg_path = Path(sys.argv[1])
script_name = sys.argv[2] if len(sys.argv) > 2 else "sim_mapping.sh"

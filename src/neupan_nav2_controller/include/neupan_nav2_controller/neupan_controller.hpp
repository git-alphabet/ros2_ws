...
protected:
  ...
  {
  public:
    explicit PyGILGuard(bool acquire = true)
    {
      if (acquire) {
        state_ = PyGILState_Ensure();
        active_ = true;
      } else {
        state_ = PyGILState_STATE{};
        active_ = false;
      }
    }
    ...
    /// Disable automatic release without changing current GIL ownership
    void disarm()
    {
      active_ = false;
    }
  ...
  double obstacle_clear_radius_;
  double min_distance_noise_floor_;

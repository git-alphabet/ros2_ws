  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  RCLCPP_INFO(
    logger_, "Obstacle clear radius: %.2f m", obstacle_clear_radius_);
  RCLCPP_INFO(
    logger_, "Min distance noise floor: %.3f m", min_distance_noise_floor_);
...
bool NeuPANController::initializePython()
{
  ...
  const bool interpreter_ready = Py_IsInitialized();
  PyGILGuard gil_guard(interpreter_ready);
  ...
  if (!actual_model_path.empty()) {
    ...
    // Overwrite pan configuration so Python uses the absolute model path
    PyDict_SetItemString(kwargs_dict, "pan", pan_dict);
    Py_DECREF(pan_dict);
  ...
  // Release the GIL so controller threads can acquire it on demand
  PyEval_SaveThread();
  gil_guard.disarm();
  ...
  geometry_msgs::msg::Twist & cmd_vel)
{
  ...
  try {
    std::function<bool(PyObject *, double &)> extract_python_scalar;
    extract_python_scalar = [&](PyObject * obj, double & value) -> bool {
      if (!obj) {
        return false;
      }
      ...
      if (PyObject_HasAttrString(obj, "item")) {
        PyObject * item = PyObject_CallMethod(obj, "item", nullptr);
        if (!item) {
          PyErr_Clear();
          return false;
        }
      ...
      if (PySequence_Check(obj)) {
        PyObject * seq = PySequence_Fast(obj, "expected sequence of scalars");
        if (!seq) {
          PyErr_Clear();
          return false;
        }
        if (PySequence_Fast_GET_SIZE(seq) == 1) {
          ...
          if (item) {
            if (PyFloat_Check(item) || PyLong_Check(item)) {
              scalar = PyFloat_AsDouble(item);
              ok = !PyErr_Occurred();
              if (!ok) {
                PyErr_Clear();
              }
            } else {
              ok = PyObject_HasAttrString(item, "item") && extract_python_scalar(item, scalar);
            }
          }
          if (ok) {
            value = scalar;
    ...
    auto extract_double_sequence = [&](PyObject * obj, std::vector<double> & values) -> bool {
      PyObject * seq = PySequence_Fast(obj, "expected sequence of numbers");
      if (!seq) {
        PyErr_Clear();
        return false;
      }
      ...
      for (Py_ssize_t i = 0; i < len; ++i) {
        ...
        if (!extract_python_scalar(item, numeric_value)) {
          Py_DECREF(seq);
          return false;
        }
        values.push_back(numeric_value);
      }
    ...
    if (stop || arrive) {
      ...
      double min_distance_value = std::numeric_limits<double>::quiet_NaN();
      bool min_distance_valid = false;
      double min_distance_interpreted = std::numeric_limits<double>::quiet_NaN();
      bool min_distance_interpreted_valid = false;
      PyObject * min_distance_obj = PyObject_GetAttrString(neupan_core_instance_, "min_distance");
      if (min_distance_obj) {
        min_distance_valid = extract_python_scalar(min_distance_obj, min_distance_value);
        if (min_distance_valid) {
          min_distance_interpreted = min_distance_value;
          if (min_distance_value < 0.0 && min_distance_value >= min_distance_noise_floor_) {
            min_distance_interpreted = 0.0;
          }
          min_distance_interpreted_valid = true;
        }
        Py_DECREF(min_distance_obj);
      } else {
        PyErr_Clear();
      }

      double collision_threshold_value = std::numeric_limits<double>::quiet_NaN();
      bool collision_threshold_valid = false;
      PyObject * collision_threshold_obj = PyObject_GetAttrString(neupan_core_instance_, "collision_threshold");
      if (collision_threshold_obj) {
        collision_threshold_valid = extract_python_scalar(collision_threshold_obj, collision_threshold_value);
        Py_DECREF(collision_threshold_obj);
      } else {
        PyErr_Clear();
      }

      bool suppressed_stop_due_to_noise = false;
      if (stop && !arrive && min_distance_valid && min_distance_value < 0.0 &&
    ...
    // Extract velocity commands from action sequence
    std::vector<double> action_values;
    if (!extract_double_sequence(action, action_values)) {
      RCLCPP_ERROR(logger_, "NeuPAN action is not a numeric sequence");
      Py_DECREF(result);
      return false;
    }
    ...
    if (robot_type_ == "omni") {
      ...
      cmd_vel.linear.y = action_values[1];
      cmd_vel.angular.z = (action_values.size() > 2) ? action_values[2] : 0.0;
    } else if (robot_type_ == "diff") {
  ...
  const geometry_msgs::msg::PoseStamped & robot_pose)
{
  ...
  const double min_range_cutoff = std::max(obstacle_clear_radius_, static_cast<double>(scan->range_min));
  size_t near_field_filtered = 0;

  for (size_t i = 0; i < scan->ranges.size(); ++i) {
    double range = scan->ranges[i];
    
    // Skip invalid measurements
    if (range < min_range_cutoff || range > scan->range_max || std::isnan(range) || std::isinf(range)) {
      if (range < min_range_cutoff) {
        ++near_field_filtered;
      }
      continue;
    }
  ...
  if (near_field_filtered > 0) {
    RCLCPP_DEBUG(
      logger_, "Filtered %zu near-field laser hits within %.2f m", near_field_filtered, min_range_cutoff);
  }

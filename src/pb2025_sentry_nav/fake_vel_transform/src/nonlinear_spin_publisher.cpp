...
public:
  ...
  : rclcpp::Node("nonlinear_spin_publisher", options)
  {
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "NonlinearSpinPublisher: enabled=%s start_on_first_trigger=%s trigger_topic=%s trigger_type=%s cmd_spin_topic=%s center=%.3f range=%.3f max_abs=%.3f update=%.3fs accel=%.3f publish=%.1fHz seed=%ld",
      enabled_ ? "true" : "false",
